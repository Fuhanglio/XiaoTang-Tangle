...
class AddCourseActivity : BaseListActivity(), ColorPickerFragment.ColorPickerDialogListener, AddCourseAdapter.OnItemEditTextChangedListener {
    ...
    /** 用户是否在取色器里手动改过颜色；改过就不再被「同名课程自动对齐」覆盖 */
    private var colorPickedByUser = false
                ...
                if (viewModel.baseBean.id == -1 || !viewModel.updateFlag) {
                    launch {
                        ...
                        if (task != null) {
                            ...
                            // 同名课程：颜色强制对齐到已有课程，避免同一门课出现两种颜色
                            if (task.color.isNotEmpty()) {
                                viewModel.baseBean.color = task.color
                            }
                        }
                        saveData(task != null)
                    }
    ...
    private fun initHeaderView(baseBean: CourseBaseBean): View {
        ...
        etName.addTextChangedListener(object : TextWatcher {
            override fun afterTextChanged(s: Editable?) {
                ...
                // 同名课程颜色自动对齐（只对新增课程生效，手动选过色则不再覆盖）
                autoMatchColor(baseBean, s.toString())
            }
    ...
    /**
     * 同名课程颜色自动对齐：输入的名称命中已有课程时，自动沿用它的颜色。
     * 只在新增课程时生效；用户手动改过颜色后不再自动覆盖。
     */
    private fun autoMatchColor(bean: CourseBaseBean, name: String) {
        if (bean.id != -1 || colorPickedByUser) return
        val typed = name.trim()
        if (typed.isEmpty()) return
        launch {
            delay(250)
            // 防抖：这段时间内名称又被改了，就放弃这次结果
            if (viewModel.baseBean.courseName.trim() != typed) return@launch
            val same = viewModel.findSameName(typed)
            if (same != null && same.color.isNotEmpty() && !colorPickedByUser) {
                applyColor(same.color, "已沿用《${same.courseName}》的颜色")
            }
        }
    }

    /** 把颜色同步到取色控件和数据模型 */
    private fun applyColor(colorStr: String, hint: String? = null) {
        val colorInt = try {
            Color.parseColor(colorStr)
        } catch (e: Exception) {
            return
        }
        val changed = viewModel.baseBean.color != colorStr
        tvColor.setTextColor(colorInt)
        tvColor.text = "点此更改颜色"
        ivColor.setColorFilter(colorInt)
        viewModel.baseBean.color = colorStr
        if (hint != null && changed) {
            Toasty.info(this, hint, Toast.LENGTH_SHORT).show()
        }
    }
